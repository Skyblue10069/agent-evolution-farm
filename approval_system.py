"""Owner approval queue and optional Flutterwave payout executor.
...
Flutterwave mobile-money payout for an approved proposal when the required
...
def execute_flutterwave(p):
    if p.get("status") != "APPROVED": raise ValueError("proposal is not approved")
    if p.get("execution_status") == "EXECUTED": return p
    key=os.getenv("FLW_SECRET_KEY", "")
    network=os.getenv("PAYOUT_MOBILE_MONEY_NETWORK", "")
    country=os.getenv("PAYOUT_COUNTRY", "CM")
    phone=p.get("recipient",{}).get("msisdn") or os.getenv("PAYOUT_MOBILE_MONEY_MSISDN", "")
    name=p.get("recipient",{}).get("name") or os.getenv("PAYOUT_RECIPIENT_NAME", "Owner")
    if not key: raise RuntimeError("FLW_SECRET_KEY is not configured")
    if not network or not phone: raise RuntimeError("PAYOUT_MOBILE_MONEY_NETWORK and recipient phone are required")
    if p.get("currency") != "XAF": raise RuntimeError("This Cameroon mobile-money payout route requires an actual XAF balance. A non-XAF earning must first have an explicit provider-reported XAF settlement/conversion record.")
    payload={"action":"instant","type":"mobile_money","payment_instruction":{
        "source_currency":"XAF","amount":{"applies_to":"destination_currency","value":int(p["amount"])},
        "recipient":{"name":{"first":name.split()[0],"last":" ".join(name.split()[1:]) or "Owner"},
        "mobile_money":{"network":network,"country":country,"msisdn":phone}},
        "destination_currency":"XAF"},"narration":p["title"][:180],"reference":p["proposal_id"]}
    req=urllib.request.Request("https://api.flutterwave.com/direct-transfers", data=json.dumps(payload).encode(), headers={"Authorization":"Bearer "+key,"Content-Type":"application/json","X-Idempotency-Key":p["proposal_id"]}, method="POST")
    try:
        with urllib.request.urlopen(req, timeout=30) as r: result=json.loads(r.read().decode())
    except urllib.error.HTTPError as e:
        raise RuntimeError(f"Flutterwave API error {e.code}: {e.read().decode()[:500]}")
    if result.get("status") != "success": raise RuntimeError(str(result))
    data=load()
    for x in data["proposals"]:
        if x["proposal_id"]==p["proposal_id"]:
            x["execution_status"]="SUBMITTED"; x["executed_at"]=now(); x["provider_transfer_id"]=(result.get("data") or {}).get("id"); x["error"]=None
            save(data); return x
    return p
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path=="/api/execute":
                pid=body["proposal_id"][0]; p=next(x for x in load()["proposals"] if x["proposal_id"]==pid); self.send_json(execute_flutterwave(p)); return
            self.send_error(404)
...
if __name__=="__main__":
    ...
    if a.cmd=="create": print(json.dumps(create_proposal(a.agent_id,a.title,a.amount,a.currency,a.reason,{"name":a.recipient_name,"msisdn":a.recipient_msisdn}),indent=2))
        ...
        p=next(x for x in load()["proposals"] if x["proposal_id"]==a.proposal_id); print(json.dumps(execute_flutterwave(p),indent=2))
