...
an append-only audit log. Failed stages print their captured output so the
GitHub Actions log shows the real reason for failure.
"""
...
STAGES = [
    ...
    ("discovery", [
        sys.executable,
        "worldwide_discovery.py",
        "--max-results", "6",
        "--max-opportunities", "1000"
    ], 570),
    ...
    ("survival", [sys.executable, "simulate.py"], 2400),
    ("coordinator", [
        sys.executable,
        "coordinator.py",
        "--batch-size", "500",
...
def run_stage(name, cmd, timeout_s):
    ...
    # Print the real error into GitHub Actions.
    if result["status"] != "ok":
        print("")
        print("=" * 70)
        print("FAILED STAGE:", name)
...
def main():
    ...
    if failures:
        ...
        print("=" * 70)
        print(
            f"AGENT EVOLUTION CYCLE {cycle}: FAILED"
        )
        print(
            f"FAILED STAGES: {failures}"
        )
        print("=" * 70)
    ...
    print(
        f"AGENT EVOLUTION CYCLE {cycle}: COMPLETE"
    )
