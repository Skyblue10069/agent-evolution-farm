...
def new_agent(used,parent=None):
    while True:
        name=AGENT_NAME_PREFIX+"-"+str(random.randrange(100000,999999))
    ...
    return {
      "id":name.lower(), "name":name, "agent_type":"agent",
...
def fresh_state():
    ...
    return {"schema_version":5,"day":0,"agents":[new_agent(used) for _ in range(POPULATION_SIZE)],
      "verified_revenue":0.0,"verified_payments":[],"dead_count":0,"alive_count":POPULATION_SIZE,
...
def apply_verified_payments(s):
    ...
    s["verified_revenue"]=round(sum(float(p.get("amount",0)) for p in payments if p.get("verified")),2)
    by_agent={}
    for p in payments:
      if p.get("verified") and p.get("agent_id"):
        by_agent[p["agent_id"]]=by_agent.get(p["agent_id"],0)+float(p.get("amount",0))
    for a in s["agents"]:
      total=round(by_agent.get(a["id"],0),2)
      a["own_verified_revenue"]=total
      a["cash_verified"]=total
    return payments
...
def rank(s):
    ...
    s["leaderboard"]=[{"rank":i,"id":a["id"],"name":a["name"],"agent_type":a.get("agent_type","agent"),
      "verified_revenue":a.get("own_verified_revenue",0),"skill_total":round(sum(a["skills"].values()),2),
...
def main():
    ...
    save(s)
    print(f"DAY {s['day']} | alive={len(s['agents'])} | permanent_deaths={s.get('dead_count',0)} | opportunities={len(opps)} | verified_today=${verified_today:.2f}")
